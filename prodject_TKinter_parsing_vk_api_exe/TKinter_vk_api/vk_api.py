from data_api_vk import OWNER_ID, TOKEN, V, URL, ALBUM_ID  # импортируем настройки
...
def writer_csv(data):  # функция записи данных в CSV формат
    ...
    with open(f'{ALBUM_ID}.csv', 'a') as f:
        writer = csv.writer(f)
...
def get_meta_data_user(data_time, user_id, comment_text_link_id, link_comment_photos, user_link):
    ...
    if user_id == 233313522:  # исключаем пользователя с ID 233313522
        pass
    else:  # передаем данные остальных пользователей
